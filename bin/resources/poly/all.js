...
}).call(this)({"events": function(exports, require, module) {// Generated by CoffeeScript 1.6.2
(function() {
...
(function() {
  ...
  initHomePage = function() {
    ...
    delDsDialog = $("#delete-data-dialog");
    $('.delete-ds').click(function(evt) {
      delDsDialog.show();
      delDsDialog.data('dsKey', $(evt.target).parents('.dash-btns').data('key'));
      return delDsDialog.css({
        top: $(evt.target).offset().top,
        left: $(evt.target).offset().left
      });
    });
    $('.delete', delDsDialog).click(function() {
      var dsKey;

      dsKey = delDsDialog.data('dsKey');
    ...
    $('.cancel', delDsDialog).click(function() {
      return delDsDialog.hide();
    });
  ...
  initDataSourceForm = function() {
    ...
    $('#new-data-source .btn-large.cancel').click(toggleDataSourceForm);
...
(function() {
  var ChartMainView, ChartViewerMainView, DashMainView, DashViewerMainView, LocalDataSource, PrecogDataSource, RemoteDataSource, _ref;
  ...
  _ref = require('poly/data/dataSource'), LocalDataSource = _ref.LocalDataSource, RemoteDataSource = _ref.RemoteDataSource, PrecogDataSource = _ref.PrecogDataSource;
  ...
  module.exports = {
    ...
    data: function(args) {
      switch (args.type) {
        ...
        case 'precog':
          return new PrecogDataSource(args.api, args.sources);
      }
    }
  };
...
(function() {
  ...
  ChartbuilderView = (function() {
    ...
    ChartbuilderView.prototype.backToDashboard = function(event) {
...
(function() {
  ...
  ChartItem = (function(_super) {
    ...
    ChartItem.prototype._makeChart = function() {
      var l, spec, _ref, _ref1, _ref2, _ref3, _ref4,
        _this = this;
...
(function() {
  var DataSource, Events, LocalDataSource, PrecogDataSource, RemoteDataSource, TOAST, serverApi,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
  ...
  PrecogDataSource = (function(_super) {
    __extends(PrecogDataSource, _super);

    function PrecogDataSource(api, sources) {
      var self;
      ...
      sources.map(function(source) {
        return self._sourcesMap[source.name] = {
          name: source.name,
          meta: source.meta,
          query: source.query,
          jsdata: polyjs.data.api(function(requestParams, callback) {
            return api.execute({
              query: source.query
            }).then(function(data) {
    ...
    PrecogDataSource.prototype.listTables = function(callback) {
      return callback(null, this._sourcesMap);
    };

    PrecogDataSource.prototype.createPolyJsData = function(tableName) {
      return this._sourcesMap[tableName].jsdata;
    };

    PrecogDataSource.prototype.getRange = function(tableName, columnName, type, callback) {
      var q;

      q = this._sourceMap[tableName].query;
      q = type === 'cat' ? "{ values : distinct((" + q + ")." + columnName + ") }" : type === 'date' ? "ds := (" + q + ") dst := if std::type::isString(ds) then std::time::parseDateTimeFuzzy(ds) else if std::type::isNumber(ds) then std::time::millisToISO(ds, \"+00:00\") else ds { min : minTime(dst), max : maxTime(dst) }" : "{ min : min((" + q + ")." + columnName + "), max : max((" + q + ")." + columnName + ") }";
      return this._api.execute({
        query: this._sourceMap[tableName].query
      }).then(function(data) {
  ...
  module.exports = {
    ...
    PrecogDataSource: PrecogDataSource
  };
...
(function() {
  ...
  HeaderView = (function() {
    function HeaderView() {
      ...
      this.exportButtonVisible = ko.observable(true);
      Events.ui.backtodbb.click.on(function() {
        _this.changeTitle("Dashboard Builder");
        return Events.nav.dashbuilder.open.trigger();
      });
      Events.nav.chartbuilder.open.on(function() {
        return _this.exportButtonVisible(false);
      });
      Events.nav.dashbuilder.open.on(function() {
        return _this.exportButtonVisible(true);
      });
    }
...
(function() {
  ...
  DashMainView = (function(_super) {
    ...
    DashMainView.prototype.onSave = function(callback) {
      return this._saveCallback = callback;
    };
